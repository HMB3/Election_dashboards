...
EMA_staffing_raw <- sqlExecute(EMA_connection,
...
EMA_staffing <- EMA_staffing_raw %>%
  mutate(AgeAtElection = floor((20211204 - DATEOFBIRTH)/10000)
...
Venue_staffing <- sqlExecute(EMA_connection,
                              "SELECT ELECTIONEVENTID,
                             REDISTRIBUTIONCODE,
                             AREACODE,
                             STAFFTYPECODE,
...
# 
# training_status <- sqlExecute(EMA_connection,
#                              "SELECT
#     electioneventid,
#     loginid,
#     typecode,
...
training_status <- sqlExecute(EMA_connection,
    ...
    ,'Y' as valueentered
FROM
    staffing.es_staffpositioncompliance sc
    left join STAFFING.ES_STAFFPOSITIONS sp on sc.areacode = sp.areacode
    and sc.electioneventid = sp.electioneventid and sp.locationid = sc.locationid and sc.positionid = sp.positionid
        where sc.ELECTIONEVENTID = ?
    and sc.typecode like 'COM027%'
...
exp_lan <- sqlExecute(EMA_connection,
                              "
SELECT
electioneventid,
loginid,
