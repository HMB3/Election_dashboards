...
CREATE VIEW [Views].[BallotTrackHistoricalStatusByDistrict]
	AS


SELECT AREA.Area_Code, BT.*, TC.TotalCons, TR.TotalRes, T2.[2.1.1] FROM [Staging].EMS_VW_EventAreaAuthorityLGWardIan AREA
LEFT JOIN (SELECT * FROM (
		SELECT CONSIGNMENT.LGACode, CONTAINERSTATUS.StatusStage, count(*) as TotalCount FROM BTDB.NSWECProd.dbo.NSW_Container CONTAINER
		...
		GROUP BY CONSIGNMENT.LGACode, CONTAINERSTATUS.StatusStage) A
		PIVOT (
					   MAX(TotalCount)
					   FOR [StatusStage] in ([1.00],
									...
									INNER JOIN
									(SELECT LGACode, SUM(ContainerQuantity) as TotalCons FROM BTDB.NSWECProd.dbo.NSW_Consignment
									WHERE VenueLongName IS NOT NULL and ContainerName = 'Carton' AND VenueName <> 'Sydney Town Hall'
									GROUP BY LGACode) TC ON TC.LGACode = BT.[LGACode]
									LEFT JOIN
									(SELECT LGACode, SUM(ContainerQuantity) as TotalRes FROM BTDB.NSWECProd.dbo.NSW_Consignment
									WHERE VenueLongName IS NOT NULL and ContainerName = 'Carton' AND VenueName <> 'Sydney Town Hall' AND ContestName like '%RESERVE%'
									GROUP BY LGACode) TR ON TR.LGAcode = BT.[LGACode]
									LEFT JOIN
