...
PRCC_connection <- odbcDriverConnect(paste0("driver={SQL Server};server=svrelecusql1;",
 ...
 PRCC_connection <- odbcDriverConnect(paste0("driver={SQL Server};server=svranalytics1;",
                                              "database=DV_Staging_PRCC_",
                                              event_group_ID,
                                              ";",
                                              "trusted_connection=true"),
                                       readOnlyOptimize = TRUE)
...
# prcc in hyperion [DV_Staging_PRCC_LG2101].
...
if (load_PRCC) {
  ...
  PRCC_raw <- sqlExecute(hyperion_connection,
                         "
                        with bulktotal as
                    (
                    SELECT 
                      ...
                      FROM [DV_Staging_PRCC_LG2101].[dbo].[GROUPBULKTOTAL]
                    ...
                    SELECT 
                      ...
                      FROM [DV_Staging_PRCC_LG2101].[dbo].[POLLINGPLACE] pp
                      left join [DV_Staging_PRCC_LG2101].[dbo].[AREAWARD] aw on pp.AREAWARD_ID = aw.AREAWARD_ID
                      left join [DV_Staging_PRCC_LG2101].[dbo].[ELECTIONAREA] ea on pp.ELECTION_AREA_ID = ea.ELECTION_AREA_ID and pp.AREAWARD_ID = ea.AREAWARD_ID
  ...
  PRCC_BPs <- sqlExecute(hyperion_connection,
                         "
...
--  ,TOTAL_PAPERS_ENTERED
  FROM [DV_Staging_PRCC_LG2101].[dbo].[BATCH] 
  group by [POLLINGPLACE_ID]
...
--  ,TOTAL_PAPERS_ENTERED
  ...
  SELECT 
      ...
      ,pp.[MULTI_WARD]
  FROM [DV_Staging_PRCC_LG2101].[dbo].[POLLINGPLACE] pp
  left join [DV_Staging_PRCC_LG2101].[dbo].[ELECTIONAREA] ea on pp.[ELECTION_AREA_ID] = ea.[ELECTION_AREA_ID]
  left join [DV_Staging_PRCC_LG2101].[dbo].[AREAWARD] aw on pp.AREAWARD_ID = aw.AREAWARD_ID
...
from batch
  ...
  PRCC_comments <- sqlExecute(hyperion_connection,
                         "
 SELECT 
ea.ELECTION_CODE
,aw.AREA_CODE
  ...
  FROM [DV_Staging_PRCC_LG2101].[dbo].[COMMENTARY] comt
  left join [DV_Staging_PRCC_LG2101].[dbo].[POLLINGPLACE] pp on comt.[POLLINGPLACE_ID] = pp.POLLINGPLACE_ID
  left join [DV_Staging_PRCC_LG2101].[dbo].[AREAWARD] aw on pp.AREAWARD_ID = aw.AREAWARD_ID
  left join [DV_Staging_PRCC_LG2101].[dbo].[ELECTIONAREA] ea on pp.ELECTION_AREA_ID = ea.ELECTION_AREA_ID and pp.AREAWARD_ID = ea.AREAWARD_ID
  ...
  DV_load_time_raw <- sqlExecute(hyperion_connection,
                      ...
                      SELECT 
      max([LoadDate])
  FROM [DV_Staging_PRCC_LG2101].[dbo].[POLLINGPLACE]
                        ",
                                 fetch=TRUE,
